package com.robotacid.engine {
    ...
    public class Level {
        ...
        public var mode3DButton:BlitButton;
        ...
        public function Level(type:int = Room.PUZZLE, dataObj:Object = null) {
            ...
            mode3DButton = uiManager.addButton(buttonXs[3], buttonY, renderer.mode3DButtonBlit, toggle3DMode, buttonRect);
            mode3DButton.active = game.mode3D;
        ...
        public function toggle3DMode():void{
            game.mode3D = !game.mode3D;
            renderer.mode3D = game.mode3D;
            mode3DButton.active = game.mode3D;
            if(game.roomPainter && game.roomPainter.palette && game.roomPainter.palette.mode3DButton){
                game.roomPainter.palette.mode3DButton.active = game.mode3D;
            }
            UserData.settings.mode3D = game.mode3D;
            UserData.push(true);
        }
        ...
        public function renderProperty(x:Number, y:Number, property:int, bitmapData:BitmapData, renderCheck:Boolean = true):void{
            ...
            if(bitmapData == renderer.bitmapData){
                ...
                if(renderer.mode3D){
                    var projPoint:Point = renderer.apply3DProjection(blit.x, blit.y);
                    blit.x = projPoint.x;
                    blit.y = projPoint.y;
                }
            } else {
                ...
                blit.y = y;
                if(renderer.mode3D){
                    var projPoint2:Point = renderer.apply3DProjection(blit.x, blit.y);
                    blit.x = projPoint2.x;
                    blit.y = projPoint2.y;
                }
            }
        ...
        public function render():void{
                    ...
                    if(
                        ...
                        if(blackOutMap && uiManager.active && blackOutMap[r][c] == 0){
                            ...
                            if(data.getCheck(c, r)){
                                ...
                                if(renderer.mode3D){
                                    var projCheckPoint:Point = renderer.apply3DProjection(renderer.checkMarkBlit.x, renderer.checkMarkBlit.y);
                                    renderer.checkMarkBlit.x = projCheckPoint.x;
                                    renderer.checkMarkBlit.y = projCheckPoint.y;
                                }
            ...
            for(i = entities.length - 1; i > -1 ; i--){
                ...
                if((property & Room.PLAYER) && checkView && !blink && data.getCheck(p.x, p.y)){
                    ...
                    if(renderer.mode3D){
                        var projCheckPoint2:Point = renderer.apply3DProjection(renderer.checkMarkBlit.x, renderer.checkMarkBlit.y);
                        renderer.checkMarkBlit.x = projCheckPoint2.x;
                        renderer.checkMarkBlit.y = projCheckPoint2.y;
                    }
                    renderer.checkMarkBlit.render(renderer.guiBitmapData);
                } else {
